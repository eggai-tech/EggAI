...
from agents.triage.classifier_v6.classifier_v6 import classifier_v6, settings
...
async def test_classifier_v6():
    ...
    # Skip test if fine-tuned model ID is not configured
    if not hasattr(settings, 'classifier_v6_model_id') or not settings.classifier_v6_model_id:
        pytest.skip("Fine-tuned model ID not configured. Set TRIAGE_CLASSIFIER_V6_MODEL_ID environment variable.")
    ...
    model_id = settings.classifier_v6_model_id
    model_name = f"{classifier_version}_{model_id.split(':')[-1] if ':' in model_id else model_id}"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.log_param("finetuned_model_id", model_id)

        # Use a smaller sample for testing to avoid high costs
        ...
        logger.info(f"Testing classifier_v6 with {len(test_dataset)} examples")
        ...
        # More lenient threshold for fine-tuned model since it might need more training
...
def test_classifier_v6_configuration():
    """Test that the classifier configuration is properly set up."""
    import os
    ...
    assert api_key is not None, "OpenAI API Key not set. Set with: export OPENAI_API_KEY='your-key-here'"
    
    # Check fine-tuned model ID
    model_id = os.getenv("TRIAGE_CLASSIFIER_V6_MODEL_ID")
    assert model_id is not None, "Fine-tuned model ID not set. Set with: export TRIAGE_CLASSIFIER_V6_MODEL_ID='ft:model-id'"
    
    # Test import
    try:
        from agents.triage.classifier_v6.classifier_v6 import (
            classifier_v6,  # noqa: F401
        )
...
@pytest.mark.asyncio 
async def test_classifier_v6_basic():
    """Basic functionality test that doesn't require a full evaluation."""
    from dotenv import load_dotenv
...
if __name__ == "__main__":
    ...
    async def run_tests():
        await test_classifier_v6_basic()
        await test_classifier_v6()
