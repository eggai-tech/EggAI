...
async def retrieve_full_document_async(
    ...
    try:
        ...
        logger.info(
            f"Successfully retrieved document {document_id}: {total_chunks} chunks, {total_chars} characters"
        )
...
def retrieve_full_document(
    ...
    try:
        ...
        if all_page_numbers:
            ...
            page_range = (
                f"{min_page}-{max_page}" if min_page != max_page else str(min_page)
            )
...
def get_document_chunk_range(
    ...
    try:
        ...
        for chunk_id in selected_chunk_ids:
            try:
                results = asyncio.run(
                    vespa_client.search_documents(query=f'id:"{chunk_id}"', max_hits=1)
                )
                if results:
