#!/usr/bin/env python3
"""
Script to drop all documents from Vespa index.

This script removes all policy documents from the Vespa search index,
...
async def get_all_document_ids(vespa_client: VespaClient) -> list[str]:
    """Get all document IDs from Vespa index."""
...
async def delete_documents_batch(vespa_client: VespaClient, document_ids: list[str]) -> dict:
    """Delete a batch of documents from Vespa using HTTP DELETE."""
    import httpx
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"HTTP client creation failed: {e}"
        }
...
async def drop_vespa_index() -> bool:
    ...
    try:
        ...
        if result["success"]:
            logger.info(f"✅ Successfully deleted all {result['deleted']} documents from Vespa index")
            return True
        else:
            logger.error(f"❌ Deletion completed with errors: {result['deleted']} deleted, {result['failed']} failed")
