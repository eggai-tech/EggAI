...
class RetrievalAPIClient:
    ...
    async def _start_api(self, port: int) -> bool:
        ...
        try:
            ...
            if not project_root:
                logger.error("Could not find project root directory with agents/ folder")
                return False
            
            process = subprocess.Popen(
                [sys.executable, "-m", "agents.policies.main"], cwd=str(project_root)
            )
    ...
    async def query_single(self, combination: ParameterCombination, test_case: RetrievalTestCase) -> RetrievalResult:
