...
class RetrievalPerformanceTester:
    ...
    def __init__(self, base_url: str = "http://localhost:8002", config: RetrievalTestConfiguration = None):
    ...
    async def stage1_query_all(self) -> List[RetrievalResult]:
        ...
        logger.info(f"🔍 Stage 1: Starting retrieval for {len(self.combinations)} combinations")
        ...
        logger.info(f"✅ Stage 1 completed: {successful}/{len(results)} queries successful")
        return results
    ...
    async def _query_parallel(self) -> List[RetrievalResult]:
        ...
        async def bounded_query(combination):
            async with semaphore:
                test_case = next((tc for tc in self.test_cases if tc.id == combination.test_case_id), None)
                if not test_case:
        ...
        return [r if not isinstance(r, Exception) else 
               RetrievalResult(self.combinations[i], [], 0.0, 0, str(r))
    ...
    async def stage2_evaluate_all(self, retrieval_results: List[RetrievalResult]) -> List[EvaluationResult]:
        """Stage 2: Evaluate all retrieval results using LLM judge."""
        logger.info(f"🧠 Stage 2: Starting LLM evaluation for {len(retrieval_results)} results")
        ...
        successful = len([r for r in results if r.error is None])
        logger.info(f"✅ Stage 2 completed: {successful}/{len(results)} evaluations successful")
        return results
    
    async def _evaluate_parallel(self, retrieval_results: List[RetrievalResult]) -> List[EvaluationResult]:
        ...
        async def bounded_evaluate(retrieval_result):
            async with semaphore:
                test_case = next((tc for tc in self.test_cases 
                                if tc.id == retrieval_result.combination.test_case_id), None)
                if not test_case:
        ...
        return [r if not isinstance(r, Exception) else 
               EvaluationResult(retrieval_results[i].combination, 0.0, 0.0, 0.0, f"Exception: {r}", False, 0.0, str(r))
               for i, r in enumerate(results)]
    
    async def _evaluate_sequential(self, retrieval_results: List[RetrievalResult]) -> List[EvaluationResult]:
        ...
        for retrieval_result in retrieval_results:
            test_case = next((tc for tc in self.test_cases 
                            if tc.id == retrieval_result.combination.test_case_id), None)
            if test_case:
                evaluation = await self.evaluator.evaluate_single(retrieval_result, test_case)
                results.append(evaluation)
                await asyncio.sleep(0.1)
        return results
    
    def stage3_report_to_mlflow(self, retrieval_results: List[RetrievalResult], evaluation_results: List[EvaluationResult]) -> None:
        """Stage 3: Report results to MLflow."""
        self.reporter.report_results(retrieval_results, evaluation_results)
    
    async def run_staged_evaluation(self) -> Tuple[List[RetrievalResult], List[EvaluationResult]]:
        """Run complete staged evaluation: Query -> Evaluate -> Report."""
        if not await self.api_client.ensure_api_running():
            raise RuntimeError(f"API at {self.base_url} is not available and could not be started")
        
        logger.info("🚀 Starting staged retrieval evaluation")
        logger.info(f"📋 {len(self.test_cases)} test cases, {len(self.combinations)} total combinations")
    ...
    def find_best_combination(self, retrieval_results: List[RetrievalResult], evaluation_results: List[EvaluationResult]) -> dict:
        ...
        for _combo_key, stats in combination_stats.items():
            ...
            stats['avg_retrieval_time'] = (sum(stats['retrieval_times']) / len(stats['retrieval_times']) 
                                         if stats['retrieval_times'] else 0)
            # Composite score: 70% quality + 20% pass rate + 10% speed bonus (lower time = higher score)
            speed_score = max(0, (100 - stats['avg_retrieval_time']) / 100) if stats['avg_retrieval_time'] > 0 else 0
            stats['composite_score'] = (0.7 * stats['avg_quality'] + 
                                      0.2 * stats['pass_rate'] + 
                                      0.1 * speed_score)
        
        # Find best combination by composite score
        if combination_stats:
            best_combo_key = max(combination_stats.keys(), key=lambda k: combination_stats[k]['composite_score'])
            return {
                'combo_key': best_combo_key,
                **combination_stats[best_combo_key]
            }
        
        return None

    def generate_summary_report(self, retrieval_results: List[RetrievalResult], evaluation_results: List[EvaluationResult]) -> str:
        ...
        if evaluation_results:
            ...
            if successful_evals:
                overall_avg_quality = sum(e.retrieval_quality_score for e in successful_evals) / len(successful_evals)
                overall_pass_rate = sum(1 for e in successful_evals if e.judgment) / len(successful_evals)
        ...
        for search_type in self.config.search_types:
            type_evals = [e for e in evaluation_results 
                         if e.combination.search_type == search_type and e.error is None]
            if type_evals:
                avg_quality = sum(e.retrieval_quality_score for e in type_evals) / len(type_evals)
                pass_rate = sum(1 for e in type_evals if e.judgment) / len(type_evals)
                type_retrievals = [r for r in retrieval_results 
                                 if r.combination.search_type == search_type and r.error is None]
                avg_time = (sum(r.retrieval_time_ms for r in type_retrievals) / len(type_retrievals) 
                           if type_retrievals else 0)
...
async def _run_retrieval_test():
    ...
    summary_report = tester.generate_summary_report(retrieval_results, evaluation_results)
    ...
    if best_combo:
        ...
        if best_combo['avg_quality'] >= 0.8:
            logger.info("✅ RECOMMENDATION: This combination provides excellent retrieval quality!")
        elif best_combo['avg_quality'] >= 0.7:
            logger.info("👍 RECOMMENDATION: This combination provides good retrieval quality.")
        else:
            logger.info("⚠️ RECOMMENDATION: Consider tuning parameters for better quality.")
    ...
    expected_combinations = len(tester.test_cases) * len(config.search_types) * len(config.max_hits_values)
    assert len(retrieval_results) == expected_combinations, f"Expected {expected_combinations} results, got {len(retrieval_results)}"
    
    successful_evals = [e for e in evaluation_results if e.error is None]
    if successful_evals:
        avg_quality = sum(e.retrieval_quality_score for e in successful_evals) / len(successful_evals)
        pass_rate = sum(1 for e in successful_evals if e.judgment) / len(successful_evals)
