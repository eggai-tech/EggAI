...
def generate_response(augmented_context: str, config: Optional[ModelConfig] = None) -> str:
...
def generate_streaming_response(
    ...
    if not augmented_context or len(augmented_context.strip()) < 10:
        ...
        async def error_stream():
            yield StreamResponse(chunk="I apologize, but I don't have enough information to provide a helpful response.")
            yield Prediction(response="I apologize, but I don't have enough information to provide a helpful response.")
        return error_stream()
    
    logger.info(f"Generating streaming response for context length: {len(augmented_context)}")
            ...
            try:
                async for chunk in chunks:
                    if isinstance(chunk, StreamResponse):
                        ...
                        final_response = chunk.response if hasattr(chunk, 'response') else ""
