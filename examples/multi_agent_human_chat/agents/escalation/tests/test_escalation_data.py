...
def test_create_ticket_success():
    ...
    # Find the new ticket
    new_ticket = next(
        (t for t in ticket_database if t["id"] == data["id"]),
        None
    )
...
def test_create_ticket_generates_unique_ids():
    ...
    result1 = create_ticket("C24680", "Technical Support", "Issue 1", "test1@example.com")
...
def test_ticket_id_format():
    ...
    result = create_ticket("FORMAT", "Technical Support", "ID format test", "format@example.com")
    data = json.loads(result)
...
def test_api_response_structure():
    ...
    required_fields = ["id", "policy_number", "department", "title", "contact_info", "created_at"]
...
def test_department_validation():
    ...
    for dept in valid_departments:
        try:
            ...
            raise AssertionError(f"Valid department {dept} should not raise exception: {e}") 