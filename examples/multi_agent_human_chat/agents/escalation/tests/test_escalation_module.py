...
def mock_escalation_tools():
    ...
    with patch('agents.escalation.dspy_modules.escalation.get_tickets_by_policy') as mock_get_tickets, \
         patch('agents.escalation.dspy_modules.escalation.create_ticket') as mock_create_ticket:
...
async def test_escalation_optimized_dspy_basic():
    ...
    mock_response.final_response = "I'll create a ticket for you to speak with a manager."
    
    async def mock_generator(*args, **kwargs):
        yield mock_response
    
    with patch('agents.escalation.dspy_modules.escalation.escalation_model') as mock_escalation:
        with patch('dspy.streamify') as mock_streamify:
            ...
            assert responses[0].final_response == "I'll create a ticket for you to speak with a manager."
...
User: My claim has been pending for weeks without any update."""
    ...
    mock_response = MagicMock()
    mock_response.final_response = "I've created an urgent escalation ticket T004 for your pending claim."
