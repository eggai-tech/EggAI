...
def test_dataset_example_types():
    ...
    for example in dataset:
        ...
        if 'payment' in chat_lower and ('through' in chat_lower or 'status' in chat_lower):
...
def test_dspy_examples_input_fields():
    ...
    billing_examples = [
        BillingExample(
            chat_history="User: Test question",
...
def test_dataset_realistic_conversations():
    ...
    for example in dataset:
        ...
        policy_found = any(policy in last_line for policy in ["A12345", "B67890", "C24680"])
        assert policy_found
...
def test_dataset_response_accuracy():
    ...
    expected_data = {
        "A12345": {"amount": 120.0, "cycle": "Monthly", "status": "Paid", "due": "2026-02-01"},
        "B67890": {"amount": 300.0, "cycle": "Quarterly", "status": "Pending", "due": "2026-03-15"},
        "C24680": {"amount": 1000.0, "cycle": "Annual", "status": "Pending", "due": "2026-12-01"}
    }
    ...
    for example in dataset:
        ...
        if policy:
            ...
            if f"${data['amount']:.2f}" in response or f"${data['amount']:.0f}" in response:
                # Amount should be correct
                assert str(data['amount']) in response or f"{data['amount']:.2f}" in response
