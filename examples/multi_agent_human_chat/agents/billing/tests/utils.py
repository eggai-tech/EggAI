...
async def wait_for_agent_response(response_queue, connection_id: str, timeout: float = 30.0) -> dict:
    ...
    while not response_queue.empty():
        try:
            ...
            logger.info(f"Cleared old response: {old_event.get('source', 'unknown')} for {old_event.get('data', {}).get('connection_id', 'unknown')}")
    ...
    while (time.perf_counter() - start_wait) < timeout:
        try:
            ...
            logger.info(f"Received event from {event.get('source', 'unknown')} for connection {event.get('data', {}).get('connection_id', 'unknown')}")
            
            # Check if this response matches our request and comes from BillingAgent
            if (event.get("data", {}).get("connection_id") == connection_id and 
                event.get("source") == "BillingAgent"):
                logger.info(f"Found matching response for {connection_id}")
                return event
            else:
                logger.info(f"Received non-matching response: {event.get('source')} for {event.get('data', {}).get('connection_id')}")
        ...
        if (time.perf_counter() - start_wait) % 5 < 0.1:
            logger.info(f"Still waiting for response from BillingAgent for connection {connection_id}...")
    
    logger.error(f"Timeout after {timeout}s waiting for response with connection_id {connection_id}")
