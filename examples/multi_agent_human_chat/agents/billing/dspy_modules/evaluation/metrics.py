...
def get_amount_score(expected: str, actual: str) -> Tuple[float, bool]:
    ...
    try:
        ...
        amount_patterns = [
            rf"\${expected_amount}",  # $300.00
            rf"\$\s*{expected_amount}",  # $ 300.00
            rf"\${expected_amount.split('.')[0]}",  # $300
            rf"\${int(float(expected_amount)):,}",  # $300 with commas if needed
        ]
        ...
        if expected_status:
            ...
            if expected_status in actual:
                ...
                for word in status_words:
                    if (
                        word.lower() == expected_status
                        or word.lower() in expected_status
                        or expected_status in word.lower()
                    ):
                        return (0.8, True)
