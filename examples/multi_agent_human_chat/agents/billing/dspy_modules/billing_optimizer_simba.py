...
def get_billing_info(policy_number: str):
    ...
    return (f'{{"policy_number": "{policy_number}", "name": "John Doe", "status": "Active", '
...
def main():
    ...
    logger.info(f"Using {len(train_set)} training examples and {len(test_set)} test examples")
    ...
    # Run optimization
    logger.info(f"Starting SIMBA optimization with minimal parameters (batch size: {batch_size})")
