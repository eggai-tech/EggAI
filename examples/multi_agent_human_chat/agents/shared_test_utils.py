...
def test_truncate_long_history_edge_cases(truncate_func, model_config_class):
    ...
    long_history = "\n".join([f"This is a much longer line {i} with more content to make it exceed the truncation length" for i in range(100)])
...
def test_signature_structure(signature_class, test_data="test"):
...
def test_signature_fields(signature_class):
...
async def test_optimized_dspy_basic(dspy_func, conversation: str, expected_response: str):
...
async def test_optimized_dspy_empty_conversation(dspy_func, expected_response: str):
...
def test_model_config_validation(model_config_class):
...
def test_truncate_long_history_with_config(truncate_func, model_config_class, agent_name: str):
...
def test_truncate_long_history_return_structure(truncate_func):
