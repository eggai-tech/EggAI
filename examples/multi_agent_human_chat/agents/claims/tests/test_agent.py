    ...
    return [
        ...
        },
        {
            ...
            "chat_messages": [
                ...
                {"role": "ClaimsAgent", "content": "What is the new address you'd like to update on your claim?"},
    ...
    while (time.perf_counter() - start_wait) < timeout:
        try:
            ...
            if (event["data"].get("connection_id") == connection_id and 
                ...
                logger.info(f"Received non-matching response from {source} for connection_id {event_conn_id}, waiting for ClaimsAgent with {connection_id}")
...
async def send_test_case(case, case_index, test_cases):
    logger.info(f"Running test case {case_index+1}/{len(test_cases)}: {case['chat_messages'][-1]['content']}")
    ...
    with mlflow.start_run(run_name=f"claims_agent_test_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"):
        ...
        for i, case in enumerate(test_cases):
            try:
                ...
                test_result, evaluation_result = await evaluate_agent_response(case, agent_response, latency_ms, i)
