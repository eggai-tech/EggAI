...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if train_size < 5:
            ...
            sys.stdout.write("\r" + " " * 60 + "\r")  # Clear the spinner
            logger.info(f"✅ Created stratified split: {len(train_set)} train, {len(test_set)} test examples")
        ...
        base_score = evaluate_with_progress(classifier_v4_program, "Evaluating baseline zero-shot model")
        ...
        try:
            ...
            def compile_with_progress():
                ...
                def show_spinner():
                    ...
                    while elapsed < 300:  # Timeout after 5 minutes (300 seconds)
                        chars = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
                        for char in chars:
                            sys.stdout.write(f"\r{char} Optimizing COPRO... (elapsed: {int(elapsed)}s)")
                            sys.stdout.flush()
                            time.sleep(0.1)
                            elapsed += 0.1
            ...
            # ------------------------------------------------------------------
            score = evaluate_with_progress(optimized_program, "Evaluating optimized model")
