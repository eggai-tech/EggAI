...
@test_agent.subscribe(channel=agents_channel, filter_func=lambda event: event.get("type") in ["billing_request", "policy_request"])
...
async def test_triage_agent():
    ...
    for case in test_cases:
        ...
        agent_type = next((key for key, value in AGENT_REGISTRY.items() if value["message_type"] == received_event["type"]), "UnknownAgent")
