...
class Settings(BaseSettings):
    ...
    prometheus_metrics_port: int = Field(
        default=9091, description="Port for Prometheus metrics server"
    )
    ...
    bootstrap_dataset_size: int = Field(
        default=30
    )  # Dataset size for v2 bootstrap optimizer (reduced for faster execution)
    ...
    test_dataset_size: int = Field(
        default=10
    )  # Limit number of test examples for quicker comparison
