...
async def dataset_detail(request: Request, dataset_id: int, db: Session = Depends(get_db)):
...
async def edit_example(request: Request, example_id: int, db: Session = Depends(get_db)):
