...
@pytest.mark.asyncio
async def test_eiffel_plus_empire(monkeypatch):
    pred = react_module(
...
@pytest.mark.asyncio
async def test_handle_balance(monkeypatch):
    pred = react_module(question="What is my balance until 2022-01-03 included?")
...
@pytest.mark.asyncio
async def test_handle_balance_plus_eiffel(monkeypatch):
    pred = react_module(
