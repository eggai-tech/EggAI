...
**No `MAXLEN` on a shared DLQ.** `retry_max_len` caps the retry streams and per-handler DLQs, not a shared `dlq_channel`. `XADD MAXLEN` trims the whole stream regardless of which writer appended, so with several services writing to one DLQ the smallest cap among them would silently delete the others' unconsumed dead letters. A shared DLQ is written untrimmed by default; retention is the sink's responsibility (`XTRIM`, or a scheduled trim once entries are processed). `RedisTransport(dlq_max_len=...)` is the opt-in hard ceiling for operators who prefer bounded memory over the risk; use the same value on every writer to that DLQ.
...
`renew_lease=True` removes the trade-off. While a handler runs, the SDK renews the entry's lease so the reclaimer skips it; a worker that crashes stops renewing, and its entries are reclaimed `retry_on_idle_ms` after the last renewal, as before:
...
|---|---|---|
| `renew_lease` | `False` | Renew in-flight entries so they are not reclaimed while being processed. Requires `retry_on_idle_ms`. |
...
**Lost leases.** If renewal finds that an in-flight entry is no longer pending for this consumer, someone else has taken responsibility for it — normally the reclaimer, after renewals failed for longer than `retry_on_idle_ms` (e.g. a Redis outage or a blocked event loop). The entry has been, or will be, redelivered. By default the running handler is then cancelled and `eggai.transport.LeaseLostError` is raised in place of its result, so it cannot overlap its redelivery; the entry is not acked (it is no longer this consumer's). With `cancel_on_lease_lost=False` an error is logged and the handler finishes (accepting the overlap). With `batch=True` the batch is one unit: if any of its entries is lost, the batch handler is cancelled and none is acked; the entries still owned stay pending and are retried. `LeaseLostError.lost_ids` names the lost ones, `message_ids` the whole batch.
